...
import { KeyRound, Clock, CheckCircle2, Search, Filter, RefreshCw, Mail, Calendar, Trash2, Sparkles, ShieldCheck } from 'lucide-react';
...
export default function OTPsList() {
  ...
  // Filter out expired OTPs automatically so they disappear immediately upon timing out
  const filteredOtps = useMemo(() => {
    return otps
      .filter(o => !isOtpExpired(o) || o.is_used) // Expired unused OTPs are removed automatically
      .filter(o => {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <KeyRound className="text-yellow-400 w-8 h-8" /> OTP Security & Live Verification Logs
          </h1>
          <p className="text-gray-400 text-sm mt-1">Live audit log of active verification codes. Expired OTPs are automatically purged and deleted.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Button 
            ...
            disabled={isPurging}
            className="bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/30 rounded-xl gap-2 text-xs"
          >
            <Trash2 className="w-4 h-4 text-red-400" /> {isPurging ? 'Purging...' : 'Purge Expired'}
          </Button>
      ...
      <div className="bg-gradient-to-r from-yellow-500/10 via-amber-500/10 to-transparent border border-yellow-500/30 p-4 rounded-2xl flex items-start gap-3 backdrop-blur-md">
        <Clock className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />
        <div className="text-xs text-yellow-200 leading-relaxed">
          <strong className="text-white font-semibold">Automatic 10-Minute Expiry & Deletion:</strong> Every verification code generated by Voyage AI is valid for exactly <strong>10 minutes</strong>. Once used, the code is archived as <span className="text-green-300 font-bold">Used & Verified</span>. If unused, as soon as the 10-minute window elapses, the code is <strong className="text-red-300 font-bold">automatically deleted and removed from the system</strong> for maximum security.
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 p-4 rounded-2xl flex flex-col md:flex-row gap-4 items-center justify-between backdrop-blur-md">
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="flex items-center bg-black/40 border border-white/10 px-3 py-2 rounded-xl text-sm">
            ...
            >
              <option value="all" className="bg-gray-800">All Active & Verified ({filteredOtps.length})</option>
              <option value="active" className="bg-gray-800">Active Only (Valid ≤ 10m)</option>
              <option value="used" className="bg-gray-800">Used & Verified Only</option>
            </select>
          </div>
        </div>
                      ...
                      >
                        ...
                        <td className="p-4 text-right">
                          <button
                            ...
                            className="p-2 rounded-lg transition-colors text-gray-400 hover:text-red-400 hover:bg-red-500/20"
